...
@section('data-page-id', 'subcategory')

@section('content')

    <div class="category">
        <section class="filter-simple display-products grid-container" id="subcategory"  data-token="{{ $token }}"  data-id="{{ $subcategory->slug }}"  >
            <div class="row column">
                <nav aria-label="You are here:" role="navigation">
                    <ul class="breadcrumbs">
                        <li><a href="/">Home</a></li>
                        <li><a href="/category/{{ $category->slug }}">{{ $category->name }}</a></li>
                        <li class="disabled">{{ $subcategory->name }}</li>
                    </ul>
                </nav>
            </div>
            ...
            <div class="button-group round">
                <div>
                    <a href="/category/{{ $category->slug }}"><button class="button  @if($_SERVER['REQUEST_URI'] !== "/category/".$slug->slug) hollow @endif  filter-simple-button is-active" data-filter="all">All</button></a>


                    @foreach($subcategories as $subcat)
                        <a href="/subcategory/{{$subcat->slug}}">
                            <button class="button @if($_SERVER['REQUEST_URI'] !== "/subcategory/".$subcat->slug) hollow @endif filter-simple-button" data-filter=" {{ $subcat->slug }} ">
                                {{ $subcat->name }}
                            </button>
                        </a>
                    @endforeach
